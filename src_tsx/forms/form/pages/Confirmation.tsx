    ...
    <div className={visible ? "flex flex-col gap-4" : "hidden"}>
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          <h2 className="text-2xl font-semibold">Dietary Requirements</h2>
          <ul>
            {data.selectedDietaryRequirements.length > 0 ? (
              data.selectedDietaryRequirements.map((key) => (
                <li key={key}>{camelCaseToString(key)}</li>
              ))
            ) : (
              <li>None</li>
          ...
          <ul>
            {data.selectedAccessibilityRequirements.length > 0 ? (
              data.selectedAccessibilityRequirements.map((key) => (
                <li key={key}>{camelCaseToString(key)}</li>
              ))
            ) : (
              <li>None</li>
